...
class FakeRouter:
    async def forecast(self, request):
        from james_runtime.financial.schemas import ForecastResult, ForecastPoint
        from datetime import datetime, timedelta, timezone

        point = ForecastPoint(
            timestamp=request.series.bars[-1].timestamp + timedelta(minutes=1),
            open=1,
            high=2,
            low=0,
            close=1,
        ...
        return ForecastResult(
            ...
            forecast=[point], generated_at=datetime.now(timezone.utc), device="test",
